...
from utils.iouloss import bbox_iou
from torch.cuda import Event
from utils.loop_matcher import LoopMatcher
from utils.wasserstein_loss import wasserstein_lossx1y1x2y2
from utils.iouloss import bbox_iou
...
class RetinaNet_D2(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.use_iou_loss             = cfg.MODEL.CUSTOM.IOU_LOSS
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            losses = self.det_loss(gt_classes, gt_deltas, box_cls, box_delta, self.focal_loss_alpha, self.focal_loss_gamma, self.cls_weights, self.reg_weights, anchors=anchors)
        ...
        for targets_per_image in targets:
            ...
            if has_gt:
                ...
                if not self.use_iou_loss:
                    ...
                    gt_anchors_reg_deltas_i = matched_gt_boxes.tensor
    ...
    def _wassentein_loss(self, pred_deltas, anchors, gt_boxes):
        with autocast(False):
            pred_boxes = self.box2box_transform.apply_deltas(pred_deltas, anchors)
            loss = wasserstein_lossx1y1x2y2(pred_boxes, gt_boxes, reduction='sum')
            return loss
    def _eiou_loss(self, pred_deltas, anchors, gt_boxes):
        with autocast(False):
            pred_boxes = self.box2box_transform.apply_deltas(pred_deltas, anchors)
            loss = bbox_iou(pred_boxes, gt_boxes, reduction='sum')
            return loss 


    def det_loss(self, gt_classes, gt_anchors_deltas, pred_logits, pred_deltas, alphas, gammas, cls_weights, reg_weights, anchors):
        ...
        all_anchor_lists = [torch.cat([x.tensor.reshape(-1,4) for _ in range(batch_size)]) for x in anchors] 
        ...
        #             g[f].detach(),
        #         ) 
        #     for w, x, a, g, f in zip(reg_weights, pred_deltas, all_anchor_lists, gt_anchors_deltas_list, foreground_idxs_list)
        #     ]

        # nwd_loss_box_reg = [
        #     w * self._wassentein_loss(
        #             x[f], 
        #             a[f].detach(),
        #             g[f].detach(),
        #         ) 
        ...
        # nwd_loss_box_reg = sum(nwd_loss_box_reg) / max(1., self.loss_normalizer)
        # eiou_loss_box_reg = sum(eiou_loss_box_reg) / max(1., self.loss_normalizer)
        # loss_nwd_eiou = 0.8 * eiou_loss_box_reg + 0.2 * nwd_loss_box_reg
        smoothl1_loss_box_reg = sum(smoothl1_loss_box_reg) / max(1., self.loss_normalizer)
        return {"loss_cls": loss_cls, "smoothl1_loss_box_reg": smoothl1_loss_box_reg}
